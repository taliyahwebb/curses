...
import { arrow, autoUpdate, flip, FloatingPortal, offset, shift, useFloating, useHover, useInteractions } from "@floating-ui/react";
...
const Tooltip: FC<PropsWithChildren<HtmlHTMLAttributes<HTMLSpanElement> & Props>> = memo(({ enable = true, children, content, body, placement = "bottom", ...props }) => {
  ...
  const { getReferenceProps, getFloatingProps } = useInteractions([useHover(context, { restMs: 0 })])

  return <>
    <span {...props} {...getReferenceProps({
      onClick: () => setOpen(false)
    })} ref={refs.setReference}>{children}</span>
    <FloatingPortal id="floating-elements">
      {enable && <AnimatePresence>
        {open && <motion.span
          ...
          className="block fixed p-1" {...getFloatingProps()} ref={refs.setFloating} style={{ zIndex: 10000, margin: 0, position: strategy, top: y ?? 0, left: x ?? 0 }}>
          <div style={
            placement === "bottom" ? {
              top: arrowY ?? 0,
              left: arrowX ?? 0
